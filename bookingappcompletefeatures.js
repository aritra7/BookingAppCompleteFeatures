...
function showUserOnList(users){
    ...
    const li = `<li class="list-group-item" id='${users._id}'>${users.username} : ${users.email} <button class="btn btn-success btn-sm" onclick=editinfo('${users.username}','${users.email}','${users.contactNo}')>Edit</button> <button class="btn btn-danger btn-sm" onclick=deleteinfo('${users._id}')>Delete</button></li>`
...
function deleteinfo(delUserInfo){
    ...
    axios.delete(`https://crudcrud.com/api/01d3733d06b14a139325aae2e5514053/AppoinmentData/${delUserInfo}`)
